...
def handler():
  ...
  name_of_project_mongodb  = sys.argv[2]
  resume_or_pause_argument = sys.argv[3]
  mongodb_public_key_decrypt = sys.argv[4]
  mongodb_private_key_decrypt = sys.argv[5]

  # Call to funtion to get secrets.
  if not mongodb_private_key_decrypt:
    try:
  ...
  for name_complete in jsonResponse['results']:
    ...
    if name_of_project_mongodb in name_complete['name']:
  ...
  if resume_or_pause_argument == 'pause':
    for group_id in dict_with_groupid_of_all_projects.values():
      req_pause_cluster_url = f"https://cloud.mongodb.com/api/atlas/v1.0/groups/{(group_id)}/clusters/{(name_of_cluster_mongodb)}"
      ...
      if(req_result_pause_cluster_url.status_code == 409) and ('Cannot pause a cluster that is already paused.' in req_result_pause_cluster_url.text):
        ...
        print ("OK Stopping MongoDB Cluster Atlas... {}".format(name_of_cluster_mongodb))
    ...
    for group_id in dict_with_groupid_of_all_projects.values():
      ...
      if(req_result_pause_cluster_url.status_code == 200):
        print ("OK Starting MongoDB Cluster Atlas... {}".format(name_of_cluster_mongodb))
      else:
